...
public class UserSvImpl implements IUserSv {
	...
	public RegisterResult registerUser(UserVo uservo) throws PaasException {
		...
		try {
			...
			logger.info("#####  createConfigInfo return：" + new Gson().toJson(configInfo));

			if (Constants.Restful.OpenServResult.SUCCESS.equals(configInfo.getResultCode())) {
				...
				if (ucInsertResult > 0) {
					...
					String adress = SystemConfigHandler.configMap.get("IPAAS-UAC.SERVICE.IP_PORT_SERVICE")
							+ SystemConfigHandler.configMap.get("IPAAS-UAC.SVWEB.URL");
					String svWebResult = HttpRequestUtil.sendPost(adress,
							"authUserId=" + userId + "&authUserName=" + userEmail + "|" + uservo.getUserPhoneNum()
									+ "&authPassword=" + uservo.getUserPwd() + "&authPid=" + pId); // pId
					JsonObject svWebJson = new Gson().fromJson(svWebResult, JsonObject.class);
	...
	public int updateUserEmailbyKey(UserVo uv) throws PaasException {
		...
		try {
			...
			String svWebResult = HttpRequestUtil.sendPost(modAdress,
					"mail=" + uv.getUserEmail() + "&userId=" + uv.getUserId());
			...
			if ("000000".endsWith(code)) { // resultCode
				...
				int count = -1;
				UserMgrOperate userMgrOperate = makeUserMgrOperateVo(uv, "0001", "11", "000000"); // uservo
																									// ,ProdId
																									// 0001/0002
																									// ,actionCode
																									// 11/12,resultCode
	...
	public String updateUserPsById(UserVo uv) throws PaasException {
		...
		try {
			...
			String svWebResult = HttpRequestUtil.sendPost(modAdress, "newPwd=" + uv.getNewPwd() + "&oldPwd="
					+ uv.getUserPwd() + "&serviceId=" + uv.getUserEmail() + "&userId=" + uv.getUserId());
			...
			if ("000000".endsWith(code)) { // resultCode
				logger.info("##### begin to insert UserMgrOperate ");
				int count = -1;
				UserMgrOperate userMgrOperate = makeUserMgrOperateVo(uv, "0002", "12", "000000"); // uservo
																									// ,ProdId
																									// 0001/0002
																									// ,actionCode
																									// 11/12,resultCode
	...
	public Object getAiEmployeeInfo(Map<String, String> paramMap) throws BusinessException {
		...
		try {
			...
			key = OaMd5Util.encryptToMD5(
					HttpRequestUtil.sendGet(addressTime, "") + "AIHRA_EMPLOYEE_INFO_FOR_HR_Vdjk*3k@-3_31");
	...
	public UserMgrOperate makeUserMgrOperateVo(UserVo uservo, String ProdId, String actionCode, String resultCode) {
		...
		if ("11".equals(actionCode)) {
			userMgrOperate.setUserOperateAction(Constants.UserMgrOperate.UserOperateAction.UserEmail); // 操作动作
																										// UserEmail
																										// 11
																										// ,UserPs12
		}
		if ("12".equals(actionCode)) {
			userMgrOperate.setUserOperateAction(Constants.UserMgrOperate.UserOperateAction.UserPs); // 操作动作
																									// UserEmail
																									// 11
																									// ,UserPs12
		}
